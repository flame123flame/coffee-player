...
const c0 = 0
@Component({
  selector: 'app-game-lotto002',
  templateUrl: './game-lotto002.component.html',
  styleUrls: ['./game-lotto002.component.css'],
})
export class GameLotto002Component implements OnInit {
  @Directive({ selector: 'img' })
  @ViewChild('num1') num1Ref: ElementRef;
  @ViewChild('num2') num2Ref: ElementRef;
  @ViewChild('num3') num3Ref: ElementRef;
  @ViewChild('numTargetE') numTargetRef: ElementRef;
  ...
  @ViewChild('showAlertModalSubmit', { static: true })
  showAlertModalSubmit: ModalPromotionsComponent;
  ...
  @ViewChild('countTime', { static: false })
  ...
  listData: any[] = [
    ...
    // { lottoName: "สามตัวหน้า", active: false, code: _3DIGIT_FRONT.code, multi: 0, max: 0, min: 0, maxNum: 0, color: "#A8BBF7", digi: 3, group: this.listType[0].group, range: this.listRange[0], isSwitch: false },
    // { lottoName: "สามตัวล่าง", active: false, code: _3DIGIT_BOT.code, multi: 0, max: 0, min: 0, maxNum: 0, color: "#A8BBF7", digi: 3, group: this.listType[0].group, range: this.listRange[0], isSwitch: false },
    {
      lottoName: 'สองตัวบน',
      active: false,
  ...
  listErrorCheck: any[] = [
    ...
    // { code: BUY_TIME_OUT, text: "รายการซื้อรอบนี้ หมดเวลาการรับแทงแล้ว" },  not use now . it use in buy2fn() now.
  ...
  isFingerHeader: Boolean[] = [false, true];
  ...
  timeClose: any;
  time: any;
  timeCheckFrist: any = true;
  isCloseAllLotto: Boolean = false
  // lottoCode: any;
  userBuyToTal: any;
  round: any;
  isPassTen: Boolean = false;
  lottoName: any;
  ...
  constructor(
    ...
    this.gameLotto.isSelect = true;
    this.round = this.route.snapshot.queryParams['code'];
    this.timeClose = this.route.snapshot.queryParams['timeClose'];
    this.timeOpen = this.route.snapshot.queryParams['timeOpen'];
    this.code = this.route.snapshot.queryParams['classCode'];
    this.lottoName = this.route.snapshot.queryParams['lottoName'];
    const serverUrl = environment.LOTTO_URL + '/socket';
    const serverUrlBean = environment.FINNBET + '/jobbeans/socket';
    const ws = new SockJS(serverUrl);
    const wsBean = new SockJS(serverUrlBean);
    this.stompClient = Stomp.over(ws);
    this.stompClientBean = Stomp.over(wsBean);
    this.time = new Date(this.timeClose).getTime() / 1000 - new Date().getTime() / 1000
    this.isCloseAllLotto = this.time <= c0 ? true : false
  ...
  ngOnInit(): void {
    ...
    this.stompClientBean.connect(
      this.username,
      'password',
      (frame) => {
        this.stompClient.debug('connected Bean  to Socket!!!');
        this.subscribeYeekee();
      },
      (error) => {
      },
      (closeEvent) => {
  ...
  public subscribeYeekee() {
    let data = {
      classCode: this.code,
      installment: moment(this.timeOpen).format('DD%2FMM%2Fyyyy'),
      round: Number(this.round),
    };
    const installment = data.installment;
    this.stompClientBean.subscribe('/UPDATE_NUMBER/' + data.classCode + "/" + installment + "/" + data.round, (message) => {
      if (message.body) {
        console.log("_________________ Socket Bean is comming!___________________");
        console.log(JSON.parse(message.body));
        this.isCloseAllLotto = this.time <= c0 ? true : false
        this.userBuyData = []
        this.userBuyData = JSON.parse(message.body);
        this.setCloseYeekeeList()
        // this.getListUserBuyResult();
      }
    });
  }
  ...
  timeFn() {
    if (this.timeCheckFrist) {
      this.timeCheckFrist = false;
    } else {
      if (!this.isCloseAllLotto) {
        this.time = c0
        this.layout.isMoney = true
        this.isCloseAllLotto = this.time <= c0 ? true : false
        this.getListUserBuy()
  ...
  getListUserBuy() {
    let data = {
      classCode: this.code,
      installment: moment(this.timeOpen).format('DD/MM/yyyy'),
      round: Number(this.round),
    };
    let path = this.isCloseAllLotto ? 'yeekee-result/get-close-result' : 'buy-yeekee/get-list-submit'
    this.httpClient
      .doPostLotto(path, data)
      .subscribe((res) => {
        if (res.status == 'SUCCESS') {
          if (!this.isCloseAllLotto) {
            this.listUserBuy = res.data;
            this.getListUserBuyResult();
          } else if (res.data) {
            this.userBuyData = []
            this.userBuyData = res.data
            this.setCloseYeekeeList()
          }
        }
      });
  ...
  setCloseYeekeeList() {
    this.listUserBuy = []
    this.listYeekeeShow = []
    if (this.userBuyData && this.userBuyData.numberResult) {
      this.userBuyData.h2 = this.userBuyData?.numberResult.toString().slice(-3)
      this.userBuyData.h3 = this.userBuyData?.numberResult.toString().slice(-5).slice(0, 2)
      this.userBuyData.h1 = this.userBuyData?.numberResult.toString().split(this.userBuyData.h3 + this.userBuyData.h2)[0]
      this.userBuyData.list = []
      this.userBuyData.list.push({ text: "ผลรวมยิงเลข", num: this.userBuyData.numberSum })
      this.userBuyData.listSeq.forEach(element => {
        this.userBuyData.list.push({ text: "สมาชิกยิงเลขได้ลำดับที่ " + element.seqOrder, num: this.numberFiveDigit(element.numberSubmit) })
      });
      this.listUserBuy = this.userBuyData.listLast20
      this.listYeekeeShow = this.userBuyData.listSeq
      this.setYeekeeShow()
    }
  }
  getListUserBuyResult() {
    let data = {
      classCode: this.code,
      installment: moment(this.timeOpen).format('DD%2FMM%2Fyyyy'),
      round: Number(this.round),
    ...
    this.httpClient
      .doGetLotto('yeekee-result/get-yeekee-round-seq?lottoClassCode=' + data.classCode + "&roundYeekee=" + data.round + "&installment=" + data.installment)
      .subscribe((res) => {
        if (res.status == 'SUCCESS') {
          this.listYeekeeShow = res.data
          this.setYeekeeShow()
        }
      });
  }
  setYeekeeShow() {
    this.listUserBuy.forEach((res, index) => {
      for (let idx = 0; idx < this.listYeekeeShow.length; idx++) {
        if (index + 1 == this.listYeekeeShow[idx].seqOrder) {
          this.listUserBuy.splice(index, 1, this.listYeekeeShow[idx])
          this.listYeekeeShow[idx].indexShow = this.listYeekeeShow[idx].seqOrder
          this.listYeekeeShow[idx].active = true
          break;
        }
      }
      res.indexShow = index + 1
    })
    this.listYeekeeShow.forEach(data => {
      if (!data.active) {
        data.indexShow = data.seqOrder
        data.active = true
        this.listUserBuy.push(data)
      }
    })
    this.listUserBuy.forEach((res, index) => {
      res.numberSubmit = this.numberFiveDigit(res.numberSubmit);
    });
    this.listUserBuy.sort((a, b) => (a.indexShow > b.indexShow) ? 1 : ((b.indexShow > a.indexShow) ? -1 : 0));
  }
  getListUserBuyToTal() {
    let data = {
      classCode: this.code,
    ...
    this.httpClient
      .doPostLotto('buy-yeekee/get-sum-submit', data)
      .subscribe((res) => {
        if (res.status == 'SUCCESS') {
          this.userBuyToTal = res.data;
        }
      });
  }
  keyupTarget(number) {
    if (this.numTarget.toString().length < 5) {
      this.numTarget = this.numTarget.toString() + number.toString();
    }
  }
  removeTargetNumber() {
    if (this.numTarget.length > 0) {
      this.numTarget = this.numTarget.substring(0, this.numTarget.length - 1);
    } else {
      this.numTarget = '';
    }
  }
  ...
  randomNumberTarget() {
    let max = 99999;
    let min = 0;
    let str = '' + Math.floor(Math.random() * (max - min + 1));
    this.numTarget = this.numberFiveDigit(str);
  }
  numberFiveDigit(num) {
    return ('00000' + num).slice(-5);
  }
  sendTargerNumber() {
    if ((null != this.numTarget && this.numTarget.length) == 5) {
      this.isPassTen = false;
      let data = {
        classCode: this.code,
        installment: moment(this.timeOpen).format('DD/MM/yyyy'),
        round: Number(this.round),
        value: Number(this.numTarget),
      };

      this.httpClient
        .doPostLotto('buy-yeekee/submit-value-sum', data)
        .subscribe((res) => {
          if (res.status == 'SUCCESS') {
            this.submitTaget.text = 'ส่งเลขสำเร็จ';
            this.submitTaget.isSuccess = true;
          } else {
            this.submitTaget.text = 'ส่งเลขไม่สำเร็จ';
            this.submitTaget.isSuccess = false;
          }
          this.getListUserBuy();
          this.getListUserBuyToTal();
          this.openShowAlertSubmit();
        });
      this.numTarget = '';
    ...
    this.httpClient
      ...
      .subscribe((res) => {
        ...
        res.data.forEach((element) => {
          if (element.lottoCategoryCode == 'YEEKEE') {
            this.listPushPoy.push(element);
          }
        });
        this.httpClient
          .doGetLotto(
            'transaction-group/get-transaction-group-by-user-show/YEEKEE'
          )
          .subscribe((res) => {
  ...
  isFingerTypeFn(index) {
    this.isFingerHeader.forEach((res, idx) => {
      this.isFingerHeader[idx] = false;
      if (index == idx) this.isFingerHeader[idx] = true;
    });
  }
  isFingerFn(index) {
    if (this.isFingerHeader[0]) {
      this.typeSelect = [];
  ...
  openShowAlertSubmit() {
    this.modalRef = this.modalService.show(this.showAlertModalSubmit, {
      class: 'modal-dialog-centered',
    });
  }
  openAlertContirmTextModal(isOpening) {
    if (isOpening) {
      this.closeModal();
    }
  ...
  typeSelectFn(item) {
    ...
    this.listData.forEach((res) => {
      if (res.active) {
        this.typeNumber += 1;
        this.digi = res.digi;    countActiveAll++
        if (res.isSwitch) {
          countActiveSwitch++
        }
      }
